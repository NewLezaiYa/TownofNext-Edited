...
internal class Admirer : RoleBase
{
    ...
    public static bool CanBeAdmired(PlayerControl target)
    {
        if ((target.IsPlayerImpostorTeam() && !CanAdmireImp.GetBool()) || (target.IsPlayerCrewmateTeam() && !CanAdmireCrew.GetBool())
            ...
            return false;
        if (AdmiredList.ContainsKey(target.PlayerId))
        {
            if (AdmiredList[target.PlayerId].Contains(target.PlayerId))
                return false;
        }
        else AdmiredList.Add(target.PlayerId, []);
